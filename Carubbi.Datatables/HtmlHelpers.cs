...
namespace Carubbi.Datatables
{
    public static class HtmlHelpers
    {
        public static MvcHtmlString Table(this HtmlHelper helper, string name, IEnumerable items, object htmlAttributes)
        {
            ...
            if (string.IsNullOrEmpty(name)) return MvcHtmlString.Empty;
        ...
        private static string BuildTable(string name, IEnumerable items, IDictionary<string, object> attributes)
        {
            ...
            var properties = itemType.GetProperties()
                ...
                .OrderBy(pr =>
                    ((DataTablesColumnAttribute) pr.GetCustomAttributes(typeof(DataTablesColumnAttribute), false)
                        .Single()).Order)
                .ToList();

            var allowEdit = attributes.ContainsKey("data-allow-edit") &&
                            Convert.ToBoolean(attributes["data-allow-edit"]);
            var allowDelete = attributes.ContainsKey("data-allow-delete") &&
                              Convert.ToBoolean(attributes["data-allow-delete"]);
            ...
            sb.AppendLine("\t<tbody>");
            foreach (var item in items) BuildTableRow(sb, item, properties);
            sb.AppendLine("\t</tbody>");
            ...
            if (allowNew)
            {
                var addButton = new TagBuilder("a");
        ...
        private static string GetKeys(List<PropertyInfo> properties)
        {
            ...
            {
                var attr = (DataTablesColumnAttribute) property
                    .GetCustomAttributes(typeof(DataTablesColumnAttribute), false).Single();
                if (attr.PrimaryKey)
        ...
        private static void BuildTableHeader(StringBuilder sb, List<PropertyInfo> properties, bool hasButtons)
        {
            ...
            foreach (var property in properties)
            {
                var attr = (DataTablesColumnAttribute) property
                    .GetCustomAttributes(typeof(DataTablesColumnAttribute), false).Single();
                sb.AppendFormat(
                    "\t\t<th data-property-name='{1}' data-property-visible='{2}' data-type='{3}'>{0}</th>\n",
